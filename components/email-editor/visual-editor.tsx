  ...
  return blocks.map(b => {
    switch (b.type) {
      case 'heading': {
        ...
        // Do NOT escape heading content — it contains Liquid variables like {{firstName}}
...
// ── Portal Menu — fixed positioning avoids all overflow clipping ──────────────
...
function PortalMenu({ pos, onSelect, onClose }: { pos: MenuPos; onSelect:(t:BlockType)=>void; onClose:()=>void }) {
  ...
  const menuHeight = 6 * 52 + 32; // approx height
  const top = pos.openUp ? pos.top - menuHeight : pos.top + 4;

  return createPortal(
    <div data-bm="1" style={{
      position: 'fixed', top, left: pos.left,
      zIndex: 9999, background: CARD, border: `1px solid ${BD}`,
...
// ── AddBlockButton — calculates position and renders portal menu ──────────────

function AddBlockButton({
  label, onSelect, style, children,
}: {
  label?: string;
  ...
  return (
    <>
      <button ref={btnRef} onClick={toggle} style={style}>
    ...
    <div style={{ display:'flex', flexDirection:'column', background:BG }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 16px', background:CARD, borderBottom:`1px solid ${BD}`, flexShrink:0 }}>
        <AddBlockButton
          ...
          style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 12px', borderRadius:7, border:`1px solid ${BD}`, background:CARD, cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:600, color:TX }}
        />
        <div style={{ flex:1 }} />
        <button
          onClick={() => setShowAI(s => !s)}
          style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 12px', borderRadius:7, border:`1px solid rgba(34,197,94,0.3)`, background:showAI?BBG:CARD, cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:600, color:BR }}
        >
      ...
      {showAI && (
        <div style={{ background:CARD, borderBottom:`1px solid ${BD}`, padding:16, flexShrink:0 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:10 }}>
            {([
              ['What you sell', aiProd, setAiProd, 'e.g. real estate course'],
              ["Who it's for",  aiAud,  setAiAud,  'e.g. beginners'],
              ['Goal',          aiGoal, setAiGoal, 'e.g. join my course'],
          ...
          <button
            onClick={handleGenerate} disabled={aiLoad}
    ...
    <div ref={setNodeRef} style={{ transform:CSS.Transform.toString(transform), transition, opacity:isDragging?0.4:1, marginBottom:6 }}>
      ...
      >
        ...
        <div {...attributes} {...listeners}
          style={{ position:'absolute', left:4, top:'50%', transform:'translateY(-50%)', cursor:'grab', color:TX3, opacity:selected?1:0, transition:'opacity 0.12s', padding:'4px 2px' }}
        ...
        {selected && (
          <div onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:8, paddingTop:8, borderTop:`1px solid ${BD}` }}>
              <button
                onClick={onDelete}
                style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 8px', borderRadius:5, border:`1px solid ${BD}`, background:'none', cursor:'pointer', fontSize:11, color:RED, fontFamily:'inherit' }}
              >
...
function BlockEdit({ block, onUpdate }: { block: Block; onUpdate: (p: Partial<Block>) => void }) {
  switch (block.type) {
          ...
          <div style={{ display:'flex', gap:5, marginBottom:8 }}>
            {([1,2,3] as const).map(l => (
              <button key={l} onClick={() => onUpdate({ level:l })}
                style={{ padding:'3px 8px', borderRadius:5, border:`1px solid ${BD}`, fontSize:11, fontWeight:700, fontFamily:'inherit', cursor:'pointer', background:block.level===l?BR:'none', color:block.level===l?'#fff':TX2 }}>
                H{l}
              </button>
            ))}
          </div>
    ...
    case 'text':
      return <VarTextarea value={block.content.replace(/<[^>]+>/g, '')} onChange={v => onUpdate({ content:v })} placeholder="Paragraph text…" />;
    case 'button':
      return (
        <div style={{ display:'flex', flexDirection:'column', gap:8, paddingTop:4 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <input type="color" value={block.color} onChange={e => onUpdate({ color:e.target.value })}
              style={{ width:32, height:28, border:`1px solid ${BD}`, borderRadius:5, cursor:'pointer', padding:2 }}/>
            <div style={{ marginLeft:'auto', display:'flex', gap:4 }}>
              {(['left','center','right'] as const).map(a => (
                <button key={a} onClick={() => onUpdate({ align:a })}
                  style={{ padding:'3px 7px', borderRadius:4, border:`1px solid ${BD}`, fontSize:10, fontFamily:'inherit', cursor:'pointer', background:block.align===a?BR:'none', color:block.align===a?'#fff':TX2 }}>
      ...
      return (
        <div style={{ display:'flex', alignItems:'center', gap:10, paddingTop:4 }}>
          ...
          <input type="range" min={8} max={120} step={4} value={block.height}
            onChange={e => onUpdate({ height:+e.target.value })} style={{ flex:1 }}/>
        </div>
      );
  }
}
...
function VarPicker({ onInsert }: { onInsert: (v: string) => void }) {
  ...
  return (
    <div data-vp="1" style={{ position:'relative', display:'inline-block' }}>
      <button
        ref={btnRef}
        type="button"
        ...
        <div style={{ position:'absolute', top:'100%', left:0, zIndex:200, marginTop:4, background:CARD, border:`1px solid ${BD}`, borderRadius:10, boxShadow:'0 4px 20px rgba(0,0,0,0.1)', padding:6, minWidth:220 }}>
          ...
          {VARS.map(v => (
            ...
            >
              ...
              <code style={{ fontSize:10, color:TX3, background:BG, padding:'2px 5px', borderRadius:4 }}>
                {v.value.replace(/\{\{|\}\}/g,'').split('|')[0].trim()}
              </code>
            </button>
          ))}
        </div>
...
// Insert text at cursor, returning new string value
// Works with React controlled inputs by using a state-based approach
...
function VarInput({ value, onChange, placeholder, autoFocus }: { value:string; onChange:(v:string)=>void; placeholder?:string; autoFocus?:boolean }) {
  ...
  function handleInsert(v: string) {
    ...
    const { value: next, cursor } = insertAtCursor(ref.current, value, v);
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:8, paddingTop:4 }}>
      <label
        ...
        style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:6, padding:'20px 16px', borderRadius:8, cursor:'pointer', border:`2px dashed ${dragging?BR:BD}`, background:dragging?BBG:BG, transition:'border-color 0.15s,background 0.15s' }}
      >
