 ...
 * Replaces the old contentEditable editor with zero additional files.
 *
 * REQUIRED: npm install @dnd-kit/core @dnd-kit/sortable @dnd-kit/utilities
 *
 * Props (same as old VisualEditor — drop-in replacement):
 *   value:      string  — current HTML
 *   onChange:   (html: string) => void
 *   brandColor: string  — workspace brand color for button default
 *   subject:    string  — campaign subject (used by AI panel)
 */
...
function defaultBlock(type: BlockType, color: string): Block {
  switch (type) {
    ...
    case 'button':  return { id: uid(), type: 'button',  text: 'Click here →', url: 'https://', color, align: 'center' };
...
const C = { bg:'#F7F8FA', card:'#FFFFFF', border:'#E6E8EC', brand:'#22C55E', brandBg:'rgba(34,197,94,0.08)', text:'#0D0F12', sub:'#5A6472', hint:'#9AA3AF', red:'#EF4444' };

// ── Block menu config ─────────────────────────────────────────────────────────

const MENU_ITEMS: { type: BlockType; label: string; icon: React.ReactNode; desc: string }[] = [
...
export function VisualEditor({ value, onChange, brandColor = '#22C55E', subject = '' }: Props) {
  const [blocks,     setBlocks]     = useState<Block[]>([]);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [addMenuAt,  setAddMenuAt]  = useState<string | 'end' | null>(null);
  ...
  const push = useCallback((fn: (prev: Block[]) => Block[]) => {
    setBlocks(prev => { const next = fn(prev); onChange(blocksToHtml(next)); return next; });
  }, [onChange]);

  function addBlock(type: BlockType, afterId: string | 'end' | null) {
    const b = defaultBlock(type, brandColor);
    push(prev => {
      if (!afterId || afterId === 'end') return [...prev, b];
      const idx = prev.findIndex(x => x.id === afterId);
      const next = [...prev]; next.splice(idx + 1, 0, b); return next;
    });
    setSelectedId(b.id); setAddMenuAt(null);
  }
  ...
  async function handleAI() {
    if (!aiProd.trim() || !aiAud.trim() || !aiGoal.trim()) { setAiError('Fill in all three fields.'); return; }
    setAiLoading(true); setAiError('');
    try {
      const { generateEmailBlocks } = await import('@/lib/ai/block-generator');
      const r = await generateEmailBlocks({ subject: subject || 'Email', product: aiProd, audience: aiAud, goal: aiGoal });
      if (!r.success) { setAiError(r.error); return; }
      const parsed = htmlToBlocks(r.data.htmlBody);
      if (parsed.length) { push(() => parsed); setShowAI(false); setSelectedId(null); }
      else setAiError('Could not parse result. Try again.');
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:C.bg }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 16px', background:C.card, borderBottom:`1px solid ${C.border}`, flexShrink:0 }}>
        <div style={{ position:'relative' }}>
          <button onClick={() => setAddMenuAt(addMenuAt === 'end' && !selectedId ? null : 'end')}
          ...
          {addMenuAt === 'end' && !selectedId && (
            <AddMenu onSelect={t => addBlock(t,'end')} onClose={() => setAddMenuAt(null)}/>
          )}
        </div>
        <div style={{ flex:1 }}/>
      ...
      {showAI && (
        <div style={{ background:C.card, borderBottom:`1px solid ${C.border}`, padding:16, flexShrink:0 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:10 }}>
            {[['What you sell',aiProd,setAiProd,'e.g. real estate course'],["Who it's for",aiAud,setAiAud,'e.g. beginners'],['Goal',aiGoal,setAiGoal,'e.g. join my course']].map(([label,val,set,ph]) => (
              <div key={label as string}>
                <label style={{ fontSize:11, fontWeight:600, color:C.sub, display:'block', marginBottom:3 }}>{label as string}</label>
                <input value={val as string} onChange={e => (set as (v:string)=>void)(e.target.value)} placeholder={ph as string} style={inp}/>
              </div>
            ))}
          </div>
          {aiError && <p style={{ fontSize:12, color:C.red, margin:'0 0 8px' }}>{aiError}</p>}
          <button onClick={handleAI} disabled={aiLoading}
            style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 16px', borderRadius:7, border:'none', background:C.brand, color:'#fff', fontSize:13, fontWeight:600, cursor:aiLoading?'wait':'pointer', opacity:aiLoading?0.7:1, fontFamily:'inherit' }}>
            <SparklesIcon size={13}/>{aiLoading?'Generating…':'Generate'}
          </button>
        </div>
      )}

      {/* Canvas */}
      <div style={{ flex:1, overflowY:'auto', padding:'16px 24px' }} onClick={() => { setSelectedId(null); setAddMenuAt(null); }}>
        <div style={{ maxWidth:560, margin:'0 auto' }} onClick={e => e.stopPropagation()}>
            ...
            <SortableContext items={blocks.map(b => b.id)} strategy={verticalListSortingStrategy}>
              {blocks.map(block => (
                <SortableBlock
                  ...
                  addMenuOpen={addMenuAt === block.id}
                  onAddMenuOpen={() => setAddMenuAt(addMenuAt === block.id ? null : block.id)}
                  onAddMenuClose={() => setAddMenuAt(null)}
                  onAddMenuSelect={t => addBlock(t, block.id)}
                />
              ))}
            </SortableContext>
          </DndContext>
          ...
          {blocks.length === 0 && (
            <div onClick={() => setAddMenuAt('end')}
              style={{ border:`2px dashed ${C.border}`, borderRadius:10, padding:'40px 20px', textAlign:'center', cursor:'pointer', color:C.hint, fontSize:13 }}>
          ...
          {blocks.length > 0 && (
            <div style={{ position:'relative', marginTop:6 }}>
              <button onClick={() => setAddMenuAt(addMenuAt === 'end' ? null : 'end')}
              ...
              {addMenuAt === 'end' && <AddMenu onSelect={t => addBlock(t,'end')} onClose={() => setAddMenuAt(null)} above/>}
            </div>
          )}
        </div>
      </div>
...
function SortableBlock({ block, selected, brandColor, onSelect, onUpdate, onDelete, addMenuOpen, onAddMenuOpen, onAddMenuClose, onAddMenuSelect }: {
  block: Block; selected: boolean; brandColor: string;
  onSelect: () => void; onUpdate: (p: Partial<Block>) => void; onDelete: () => void;
  addMenuOpen: boolean; onAddMenuOpen: () => void; onAddMenuClose: () => void;
  onAddMenuSelect: (t: BlockType) => void;
}) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.4 : 1, marginBottom:6, position:'relative' }}>
      <div onClick={onSelect} style={{ position:'relative', borderRadius:8, border: selected ? `2px solid ${C.brand}` : `1px solid transparent`, background: selected ? C.brandBg : 'transparent', padding:'2px 4px 2px 26px', cursor:'pointer', transition:'border-color 0.12s,background 0.12s' }}
        onMouseEnter={e => { if (!selected) (e.currentTarget as HTMLElement).style.borderColor = C.border; }}
        onMouseLeave={e => { if (!selected) (e.currentTarget as HTMLElement).style.borderColor = 'transparent'; }}>
        <div {...attributes} {...listeners} style={{ position:'absolute', left:4, top:'50%', transform:'translateY(-50%)', cursor:'grab', color:C.hint, opacity: selected ? 1 : 0, transition:'opacity 0.12s', padding:'4px 2px' }} onClick={e => e.stopPropagation()}>
          <GripVerticalIcon size={12}/>
        </div>
        {!selected && <Preview block={block}/>}
        {selected && (
          <div onClick={e => e.stopPropagation()}>
            <Edit block={block} brandColor={brandColor} onUpdate={onUpdate}/>
            <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:8, paddingTop:8, borderTop:`1px solid ${C.border}` }}>
              <button onClick={onDelete} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 8px', borderRadius:5, border:`1px solid ${C.border}`, background:'none', cursor:'pointer', fontSize:11, color:C.red, fontFamily:'inherit' }}>
                <TrashIcon size={11}/> Delete
              </button>
              <div style={{ flex:1 }}/>
              <div style={{ position:'relative' }}>
                <button onClick={onAddMenuOpen} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 8px', borderRadius:5, border:`1px solid ${C.border}`, background:'none', cursor:'pointer', fontSize:11, color:C.sub, fontFamily:'inherit' }}>
                  <PlusIcon size={11}/> Add below
                </button>
                {addMenuOpen && <AddMenu onSelect={onAddMenuSelect} onClose={onAddMenuClose}/>}
              </div>
            </div>
          </div>
        )}
...
function Preview({ block }: { block: Block }) {
  switch (block.type) {
    case 'heading':
      return <div style={{ fontFamily:"'Bricolage Grotesque',system-ui,sans-serif", fontSize: block.level===1?22:block.level===2?18:15, fontWeight:700, color:C.text, padding:'6px 0', lineHeight:1.2 }}>{block.content || <span style={{ color:C.hint }}>Heading…</span>}</div>;
    case 'text':
      return <div style={{ fontSize:14, color:C.sub, padding:'5px 0', lineHeight:1.6, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} dangerouslySetInnerHTML={{ __html: block.content || `<span style="color:${C.hint}">Paragraph…</span>` }}/>;
    case 'button':
      return <div style={{ textAlign:block.align, padding:'8px 0' }}><span style={{ display:'inline-block', background:block.color, color:'#fff', padding:'8px 20px', borderRadius:7, fontSize:13, fontWeight:600 }}>{block.text||'Button'}</span></div>;
    case 'image':
      return <div style={{ textAlign:'center', padding:'8px 0' }}>{block.src ? <img src={block.src} alt={block.alt} style={{ maxWidth:'100%', maxHeight:100, borderRadius:6, objectFit:'cover' }}/> : <div style={{ height:56, border:`1px dashed ${C.border}`, borderRadius:6, display:'flex', alignItems:'center', justifyContent:'center', color:C.hint, fontSize:12, gap:6 }}><ImageIcon size={14}/> Image URL not set</div>}</div>;
    case 'divider':
      return <hr style={{ border:'none', borderTop:`1px solid ${C.border}`, margin:'10px 0' }}/>;
    case 'spacer':
      return <div style={{ height:Math.min(block.height,36), display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, color:C.hint, gap:4 }}><MoveVerticalIcon size={12}/>{block.height}px spacer</div>;
  }
}

// ── Edit ──────────────────────────────────────────────────────────────────────

function Edit({ block, brandColor, onUpdate }: { block: Block; brandColor: string; onUpdate: (p: Partial<Block>) => void }) {
  const inp: React.CSSProperties = { width:'100%', padding:'8px 10px', fontSize:13, border:`1px solid ${C.border}`, borderRadius:6, outline:'none', fontFamily:'inherit', color:C.text, background:C.card, boxSizing:'border-box' };
  switch (block.type) {
    case 'heading':
      return <div style={{ paddingTop:4 }}>
        <div style={{ display:'flex', gap:5, marginBottom:8 }}>
          {([1,2,3] as const).map(l => <button key={l} onClick={() => onUpdate({ level:l })} style={{ padding:'3px 8px', borderRadius:5, border:`1px solid ${C.border}`, fontSize:11, fontWeight:700, fontFamily:'inherit', cursor:'pointer', background:block.level===l?C.brand:'none', color:block.level===l?'#fff':C.sub }}>H{l}</button>)}
        </div>
        <input autoFocus value={block.content} onChange={e => onUpdate({ content:e.target.value })} style={inp} placeholder="Heading text…"/>
      </div>;
    case 'text':
      return <textarea autoFocus value={block.content.replace(/<[^>]+>/g,' ')} onChange={e => onUpdate({ content:e.target.value })} rows={3} style={{ ...inp, resize:'vertical', height:'auto', lineHeight:1.6 }} placeholder="Paragraph text…"/>;
    case 'button':
      return <div style={{ display:'flex', flexDirection:'column', gap:8, paddingTop:4 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <input type="color" value={block.color} onChange={e => onUpdate({ color:e.target.value })} style={{ width:32, height:28, border:`1px solid ${C.border}`, borderRadius:5, cursor:'pointer', padding:2 }}/>
          <button onClick={() => onUpdate({ color:brandColor })} style={{ fontSize:11, color:C.brand, background:'none', border:'none', cursor:'pointer', fontFamily:'inherit' }}>Reset</button>
          <div style={{ marginLeft:'auto', display:'flex', gap:4 }}>
            {(['left','center','right'] as const).map(a => <button key={a} onClick={() => onUpdate({ align:a })} style={{ padding:'3px 7px', borderRadius:4, border:`1px solid ${C.border}`, fontSize:10, fontFamily:'inherit', cursor:'pointer', background:block.align===a?C.brand:'none', color:block.align===a?'#fff':C.sub }}>{a[0].toUpperCase()}{a.slice(1)}</button>)}
          </div>
        </div>
      </div>;
    case 'image':
    ...
    case 'divider':
      return <p style={{ fontSize:12, color:C.hint, padding:'4px 0', margin:0 }}>Horizontal divider — no settings.</p>;
    case 'spacer':
      return <div style={{ display:'flex', alignItems:'center', gap:10, paddingTop:4 }}>
...
function AddMenu({ onSelect, onClose, above }: { onSelect:(t:BlockType)=>void; onClose:()=>void; above?: boolean }) {
  ...
  return (
    <div data-addmenu="true" style={{ position:'absolute', [above?'bottom':'top']:'100%', left:0, zIndex:100, marginTop:4, marginBottom:4, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, boxShadow:'0 4px 20px rgba(0,0,0,0.1)', padding:6, minWidth:220 }}>
