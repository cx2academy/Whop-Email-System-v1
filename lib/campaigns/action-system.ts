...
async function executeSendEmail(
  ...
  // Full personalization using shared variable parser
  const { parseVariables, buildSendVariables } = await import('@/lib/templates/variable-parser');
  ...
  const subject = parseVariables(config.subject, vars);
  const html    = parseVariables(config.html, vars);
