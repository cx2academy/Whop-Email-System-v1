 ...
 *
 * Dashboard home — workspace overview with key stats.
 * Protected by the dashboard layout (requireWorkspaceAccess called there).
 */
...
export default async function DashboardPage() {
  ...
  const [workspace, contactCount, campaignCount, recentCampaigns] =
    await Promise.all([
      db.workspace.findUnique({
        where: { id: workspaceId },
        select: { name: true, plan: true, monthlyEmailCap: true },
      }),
      db.contact.count({
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          description={`${formatNumber(workspace?.monthlyEmailCap ?? 0)} emails/mo`}
          icon="⚡"
        />
      </div>
