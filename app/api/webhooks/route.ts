...
async function verifyWhopSignature(
  ...
  // SECURITY: Never skip verification — a missing secret means the endpoint
  // is unprotected and anyone can forge events. Reject until it's configured.
  if (!secret) {
    // Allow bypass ONLY in local development to ease initial setup
    if (process.env.NODE_ENV === 'development') {
      console.warn(
        '[whop/webhook] ⚠️  WHOP_WEBHOOK_SECRET not set — bypassing verification in development only. ' +
        'Set this env var before deploying to production.'
      );
      return { valid: true };
    }
    console.error('[whop/webhook] ❌ WHOP_WEBHOOK_SECRET not configured — rejecting request.');
    return { valid: false, reason: 'Webhook secret not configured on this server' };
  }

  const signature = req.headers.get('x-whop-signature');
  if (!signature) return { valid: false, reason: 'Missing x-whop-signature header' };
  ...
  if (signature.length !== expected.length) return { valid: false, reason: 'Signature length mismatch' };
  let diff = 0;
...
export async function POST(req: NextRequest) {
  ...
  // Verify signature
  const sigResult = await verifyWhopSignature(req, rawBody);
  if (!sigResult.valid) {
    console.warn(`[whop/webhook] Rejected: ${sigResult.reason}`);
    return Response.json({ error: sigResult.reason ?? 'Invalid signature' }, { status: 401 });
  }
